...
SdfPathVector HairProcHairProceduralSceneIndex::GetChildPrimPaths(const SdfPath& primPath) const
{
    ...
    if (entry.primType == HdPrimTypeTokens->basisCurves)
    {
      ...
      else if (curveSchema && primvarSchema)
      {
        ...
        if (primPv && uvPv && restPv && targetPv)
        {
          ...
          {
            _init_deformer(
              entry.primPath, syntheticSchema, curveSchema, primvarSchema, augmentedSource);
          }
        }
      }
    }
  ...
  const HdSceneIndexBase& sender, const HdSceneIndexObserver::DirtiedPrimEntries& entries)
{
  ...
  // If any prims in entries are part of _targets, we need to also dirty their sources, ie the
  // hairProcedural prims
  if (!_IsObserved())
  {
    return;
  }
...
void HairProcHairProceduralSceneIndex::_init_deformer(const SdfPath& primPath,
  HairProcHairProceduralSchema& procSchema, HdBasisCurvesSchema& basisCurvesSchema,
  HdPrimvarsSchema& primvarSchema, HdContainerDataSourceHandle sourceDs)
{
  ...
  HairProcHairProceduralDeformerSharedPtr deformer =
    std::make_shared<HairProcHairProceduralDeformer>(targetDs, sourceDs, primPath);
