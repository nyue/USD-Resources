  ...
  const HdSampledDataSource::Time& shutterOffset)
{
  ...
  procKernel->SetBufferData<float>(
    tgtPos.data()->data(), "tgtPos", tgtPos.size() * sizeof(GfVec3f));
  procKernel->SetBufferData<float>(
    tgtFrames.data()->data(), "frames", tgtFrames.size() * sizeof(GfMatrix3f));
...
bool HairProcHairProceduralDeformer::InitOCL()
{
  ...
  peasyocl::KernelHandle* procKernel =
    _oclContext->AddKernel(clCode, {}, "HairProc", _primPath + "HairProc");
  peasyocl::KernelHandle* tgtKernel =
    _oclContext->AddKernel(clCode, {}, "CalcTargetFrames", _primPath + "TargetFrames");
  ...
  err = tgtKernel->AddArgument<float>(
    CL_MEM_READ_WRITE, "tgtPos", tgtPos.size() * sizeof(GfVec3f), (float*)nullptr);
  err |= tgtKernel->AddArgument<int>(
    CL_MEM_READ_ONLY, "tgtIndices", tgtPrimIndices.size() * sizeof(int), tgtPrimIndices.data());
  err |= tgtKernel->AddArgument<int>(
    CL_MEM_READ_ONLY, "tgtLengthsa", tgtPrimLengths.size() * sizeof(int), tgtPrimLengths.data());
  err |= tgtKernel->AddArgument<int>(
    CL_MEM_READ_ONLY, "tgtOffset", tgtPrimOffset.size() * sizeof(int), tgtPrimOffset.data());
  err |= tgtKernel->AddArgument<float>(
    CL_MEM_READ_ONLY, "tgtXform", sizeof(GfMatrix4f), (float*)nullptr);
  err |= tgtKernel->AddArgument<int>(
    CL_MEM_READ_ONLY, "unique_prims", _uniquePrims.size() * sizeof(int), _uniquePrims.data());
  err |= tgtKernel->AddArgument<float>(
    CL_MEM_WRITE_ONLY, "result", _uniquePrims.size() * sizeof(GfMatrix3f), (float*)nullptr);
  int invert_val = 0;
  ...
  err = procKernel->AddArgument<float>(
    CL_MEM_WRITE_ONLY, "result", srcPos.size() * sizeof(GfVec3f), (float*)nullptr);
  err |= procKernel->AddArgument<float>(
    CL_MEM_READ_ONLY, "srcPos", srcPos.size() * sizeof(GfVec3f), srcPos.data()->data());
  err |= procKernel->AddArgument<int>(
    CL_MEM_READ_ONLY, "srcLengths", srcPrimLengths.size() * sizeof(int), srcPrimLengths.data());
  err |= procKernel->AddArgument<int>(
    CL_MEM_READ_ONLY, "srcIndices", srcPrimIndices.size() * sizeof(int), srcPrimIndices.data());

  err |= procKernel->AddArgument<float>(
    CL_MEM_READ_ONLY, "tgtPos", tgtPos.size() * sizeof(GfVec3f), (float*)nullptr);
  err |= procKernel->AddArgument<int>(
    CL_MEM_READ_ONLY, "tgtLengths", tgtPrimLengths.size() * sizeof(int), tgtPrimLengths.data());
  err |= procKernel->AddArgument<int>(
    CL_MEM_READ_ONLY, "tgtIndices", tgtPrimIndices.size() * sizeof(int), tgtPrimIndices.data());
  err |= procKernel->AddArgument<int>(
    CL_MEM_READ_ONLY, "tgtOffsets", tgtPrimOffset.size() * sizeof(int), tgtPrimOffset.data());

  err |= procKernel->AddArgument<float>(CL_MEM_READ_ONLY, "restFrames",
    _uniquePrims.size() * sizeof(GfMatrix3f), targetRestFrames.data()->data());
  err |= procKernel->AddArgument<float>(
    CL_MEM_READ_ONLY, "frames", _uniquePrims.size() * sizeof(GfMatrix3f), (float*)nullptr);

  err |= procKernel->AddArgument<float>(
    CL_MEM_READ_ONLY, "captUv", captUv.size() * sizeof(GfVec2f), captUv.data()->data());
  err |= procKernel->AddArgument<int>(
    CL_MEM_READ_ONLY, "captPrim", _sortedCaptPrims.size() * sizeof(int), _sortedCaptPrims.data());
  err |= procKernel->AddArgument<int>(
    CL_MEM_READ_ONLY, "uniquePrims", _uniquePrims.size() * sizeof(int), _uniquePrims.data());
...
VtMatrix3fArray HairProcHairProceduralDeformer::_CalcTargetFrames(
  const HdSampledDataSource::Time& shutterOffset, const bool invert, VtVec3fArray& pts,
  GfMatrix4f& xform)
{
  peasyocl::Context* _oclContext = peasyocl::Context::GetInstance();
