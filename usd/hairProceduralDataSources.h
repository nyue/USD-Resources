...
private:
  _PrimvarDataSource(const HdDataSourceBaseHandle& primvarValueSrc, const TfToken& interpolation,
    const TfToken& role)
...
public:
  ...
  bool GetContributingSampleTimesForInterval(
    const Time startTime, const Time endTime, std::vector<Time>* const outSampleTimes) override;

private:
  _PointsDataSource(
    HdPrimvarsSchema& primvarSchema, HairProcHairProceduralDeformerSharedPtr deformer)
...
private:
  _HairProcDataSource(const SdfPath& primPath, const HdContainerDataSourceHandle& primDataSource,
    HairProcHairProceduralDeformerSharedPtr deformer)
    : _primPath(primPath)
    , _primDs(primDataSource)
    , _deformer(deformer)
  {
  }
